...
- `Walk`: has an `int` representing the number of miles to walk as associated data.
...
- Take a walk: yes if the walk is less then three miles; maybe if it is less than five miles; otherwise, no.
